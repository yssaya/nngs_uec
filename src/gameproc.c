  ...
  case END_DONE:
    ...
    */
    ...
    pcn_out(pb, CODE_INFO, FORMAT_s_HAS_RESIGNED_THE_GAME_n,
           ...
           parray[pb].pname : parray[pw].pname);
    */
    sprintf(tmp, "ended. W %.lf B %.lf}"/*" %s resigns. W %.1f B %.1f}\n"*/,
  ...
  /* Play the move, test if valid */
    ...
    if (!strcmp(command, "pass")) {    /* User passes */
      ...
      if (garray[g1].teach != 1 && garray[g1].num_pass >= 2) {
        ...
        pcn_out_prompt(pw, CODE_RESULT, FORMAT_YOU_CAN_CHECK_YOUR_SCORE_WITH_THE_SCORE_COMMAND_TYPE_DONE_WHEN_FINISHED_n);	
        /*pcn_out_prompt(pb, CODE_INFO, FORMAT_YOU_CAN_CHECK_YOUR_SCORE_WITH_THE_SCORE_COMMAND_TYPE_DONE_WHEN_FINISHED_n);*/
	pcn_out_prompt(pb, CODE_RESULT, FORMAT_YOU_CAN_CHECK_YOUR_SCORE_WITH_THE_SCORE_COMMAND_TYPE_DONE_WHEN_FINISHED_n);	
      }
    }
...
*/
  ...
  Logit("bname=%s,wname=%s,pb=%d,pw=%d\n",bname,wname,pb,pw);

  if (p == pb) p2 = pw;
  else if (p == pw) p2 = pb;
  else {
    if ( p >= 0 && p < PARRAY_SIZE && parray[p].adminLevel >= ADMIN_ADMIN ) {
      p1 = pw;
      p2 = pb;
      Logit("admin p=%d can load any game.\n",p);
    } else {
      pcn_out(p, CODE_ERROR, FORMAT_YOU_CANNOT_LOAD_SOMEONE_ELSE_S_GAME_TRY_qLOOKq_TO_SEE_THE_GAME_);
      return COM_OK;
    }
  ...
  if (!parray[p2].flags.is_open || (p1 >= 0 && !parray[p1].flags.is_open) ) {
  ...
  pcn_out(p2, CODE_INFO, FORMAT_s_HAS_RESTARTED_YOUR_GAME_n, 
          parray[p].pname);
  if ( p1 >= 0 ) { /* send restart to both players */
