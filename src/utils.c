...
int pcommand(int p, const char *comstr, ...)
{
  ...
  retval = process_input(parray[p].session.socket, buff);
  if (retval == COM_LOGOUT) {
    process_disconnection(parray[p].session.socket);
    /* net_close(fd); */
  }
  return retval;
}
...
int pcn_out_prompt(int p, int code, int num,...)
{
  ...
  char buf[(19+2)*(19+2)];
  int i = 0;
  int s = player_find_fd(parray[p].session.socket);
  va_start(ap, num);
