...
static int com_xadminmatch(int p, struct parameter * param, int gametype);
...
int com_gadminmatch(int p, struct parameter * param)
{
  return com_xadminmatch(p, param, GAMETYPE_GO);
}
...
static int com_xadminmatch(int p, struct parameter * param, int gametype)
{
  ...
  */
  stolower(param[0].val.word);
  p0 = player_find_part_login(param[0].val.word);
  ...
  wp = (bp == p0) ? p1 : p0;
  */
  if ((size = param[3].val.integer) < 2)
  {
    pcn_out(bp, CODE_ERROR, FORMAT_BOARDSIZE_MUST_BE_2n);
    return COM_OK;
  }
  if ((start_time = param[4].val.integer) < 0)
  {
    pcn_out(bp, CODE_ERROR, FORMAT_START_TIME_MUST_BE_0n);
    return COM_OK;
  }
  switch(gametype) {
  case GAMETYPE_GO:
  case GAMETYPE_TNETGO:
    if ((byo_time = param[5].val.integer) < 0) {
      pcn_out(bp, CODE_ERROR, FORMAT_BYO_YOMI_TIME_MUST_BE_0n);
      return COM_OK;
    }
    break;
  default:
  ...
  if (!mine) {
    mine = player_pending_new(bp, wp, pendtype);
    if (!mine) {
      pcn_out(p0, CODE_ERROR, FORMAT_SORRY_YOU_CAN_T_HAVE_ANY_MORE_PENDING_MATCHES_);
      return COM_OK;
    }
  ...
  if (/*hers*/ /* Identical match, should accept! */
    ...
    if (create_new_gomatch(wp, bp,
        size, start_time, byo_time, 0, rulestype, gametype) == COM_FAILED) {
        pcn_out(bp, CODE_ERROR, FORMAT_THERE_WAS_A_PROBLEM_CREATING_THE_NEW_MATCH_);
        pcn_out_prompt(wp, CODE_ERROR, FORMAT_THERE_WAS_A_PROBLEM_CREATING_THE_NEW_MATCH_n);
    }
    /* Logit("Set Game_TYPE to %d(p=%d,r=%d)", gametype,pendtype, rulestype); */
    parray[bp].match_type = gametype;
    parray[wp].match_type = gametype;
    player_remove_requests(bp, wp, pendtype);
    player_remove_requests(wp, bp, pendtype);
    return COM_OK;
  }
  ...
  case 0:
    break;
  case 1:
    pcn_out(bp,CODE_INFO, FORMAT_UPDATING_OFFER_ALREADY_MADE_TO_qsq_n,
            parray[wp].pname);
    pcn_out(wp, CODE_INFO, FORMAT_s_UPDATES_THE_MATCH_REQUEST_n,
            parray[bp].pname);
    break;
  case 2:
  case 3:
    pcn_out(bp,CODE_INFO, FORMAT_DECLINING_OFFER_FROM_s_AND_OFFERING_NEW_MATCH_PARAMETERS_n, parray[wp].pname);
    pcn_out(wp, CODE_NONE, FORMAT_s_DECLINES_YOUR_REQUEST_FOR_A_MATCH_n, parray[bp].pname);
    break;
  }
  /*
  pcn_out(bp, CODE_INFO, FORMAT_REQUESTING_MATCH_IN_d_MIN_WITH_s_AS_s_n,
		start_time,
                parray[wp].pname,
                "White");
  /*
  pcn_out(wp, CODE_INFO, FORMAT_REQUESTING_MATCH_IN_d_MIN_WITH_s_AS_s_n,
		start_time,
                parray[bp].pname,
                "Black");
  */

  pcn_out(bp, CODE_INFO, FORMAT_MATCH_dXd_IN_d_MINUTES_REQUESTED_WITH_s_AS_s_n,
                 size, size,
                 start_time,
                 parray[wp].pname,
                 "White");
  pcn_out(wp, CODE_INFO, FORMAT_MATCH_dXd_IN_d_MINUTES_REQUESTED_WITH_s_AS_s_n,
                 size, size,
                 start_time,
                 parray[bp].pname,
                 "Black");

  pcn_out_prompt(bp, CODE_INFO, formattype,
                 parray[wp].pname,
                 "B",
                 size,
                 start_time,
                 byo_time,
                 parray[wp].pname);
  pcn_out_prompt(wp, CODE_INFO, formattype,
                 parray[bp].pname,
                 "W",
                 size,
                 start_time,
...
#else
#define KOMI19 6.5 /*standard komi = 6.5 => one stone is worth 11 points
...
#else
#define KOMI13 6.5 /* From the Go FAQ. (Note BTW that the table in the FAQ
...
#else
#define KOMI9  7.0 /* 5.5 according to some pro-pro games. */
#endif
